...
class CognitoStack(Stack):
    ...
    def _create_user_pool(self) -> cognito.UserPool:
        ...
        sms_role = None
        if (self.config.mfa != cognito.Mfa.OFF and 
            self.config.mfa_second_factor.get("sms", False)):
            ...
            sms_role = iam.Role(
                ...
                managed_policies=[
                    iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AmazonCognitoIdpSMSRole")
                ]
            )
        ...
        user_pool = cognito.UserPool(
            ...
            removal_policy=RemovalPolicy.RETAIN if self.config.deletion_protection else RemovalPolicy.DESTROY,
            sms_role=sms_role,
        )
    ...
    def _create_user_pool_domain(self) -> cognito.UserPoolDomain:
        ...
        if not domain_prefix:
            # Use user pool ID without underscores and in lowercase, plus add a suffix to ensure uniqueness
            import re
            base_prefix = re.sub(r'[^a-z0-9-]', '', self.user_pool.user_pool_id.lower().replace('_', '-'))
            domain_prefix = f"agentcore-{base_prefix[:10]}"
    ...
    def _create_user_pool_client(self) -> cognito.UserPoolClient:
        ...
        oauth_scopes = []
        for scope_config in self.config.scopes:
            scope_name = f"{self.config.resource_server_identifier}/{scope_config.scope_name}"
    ...
    def _create_client_secret_custom_resource(self) -> None:
        ...
        self.client_secret = secretsmanager.Secret.from_secret_name_v2(
            self,
            "ClientSecret",
            secret_name=self.output_config.client_secret_name
        )
